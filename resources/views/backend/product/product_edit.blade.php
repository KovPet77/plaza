            ...
            <div class="border border-3 p-4 rounded">
              <div class="row g-3">
                <div class="col-md-6">
                  ...
                  <div class="col-12">
                    <label for="inputCollection" class="form-label">Eladó választása</label>
                    <select name="vendor_id" class="form-select" id="inputCollection">
                        <option></option>
                        @foreach($activeVendor as $vendor)
                       <option value="{{$vendor->id}}" {{ $vendor->id == $products->vendor_id ? 'selected' : ''}}>{{$vendor->name}}</option>  <!-- Itt a user adatbázisból a name és az id mező van kiszedve -->
                        @endforeach
                      </select>
                  </div>
                  ...
                  <form method="POST" action="{{route('update.product.multiimage')}}" enctype="multipart/form-data" >
                   ...
                   <tr>
                     ...
                     <input type="submit" class="btn btn-primary" value="Fotó frissítése" />
                      <a href="{{ route('product.multiimage.delete', $img->id)}}" class="btn btn-danger" id="delete">Törlés</a>
                    </td>
										</tr>
